"""
Milestone-3: Agentic Controller for Few-Shot Adaptation (Omniglot)

Agent behavior:
1) Compute embeddings.
2) Compute Prototype logits ALWAYS.
3) If Prototype confidence is high => choose Prototype (fast + stable).
4) Else compute Ridge logits and compare proxies vs Prototype.
5) Choose Ridge only if it appears better (confidence gain, entropy not worse, low disagreement).
6) Sentinel safety: if Ridge chosen but violates drift/latency -> rollback to Prototype.

Outputs:
- CSV with per-episode method choice + metrics
- Plots
"""
...
def main():
    ...
    ap.add_argument("--out_dir", type=str, default="./runs/m3_agent")
    ...
    # Agent thresholds (start values)
    ap.add_argument("--proto_conf_high", type=float, default=0.62)   # if proto already confident -> don't try ridge
    ap.add_argument("--ridge_conf_gain", type=float, default=0.05)   # ridge must improve confidence by this
    ap.add_argument("--max_entropy_gap", type=float, default=0.20)   # ridge entropy not much worse than proto
    ap.add_argument("--max_js", type=float, default=0.15)            # ridge not wildly disagree with proto
    ...
    for ep in range(1, args.episodes + 1):
        ...
        with torch.no_grad():
            ...
            # Always compute prototype first
            ...
            # Agent decision: start with prototype
            chosen = "prototype"
            logits_chosen = logits_proto

            # If prototype not confident, try ridge (especially useful for higher-shot)
            if conf_proto < args.proto_conf_high and args.shots >= 3:
                ...
                # Choose ridge only if it looks better
            ...
            # Sentinel safety check (hard constraints)
            alerts = []
        ...
        if ep % max(1, args.episodes // 10) == 0:
            print(
                ...
                f"chosen={chosen} -> final={final_method} | latency={latency_total_ms:.2f}ms"
            )

    df = pd.DataFrame(rows)
    csv_path = out_dir / "episode_metrics_m3_agent.csv"
    ...
    print("  ridge_used_rate:", f"{ridge_used/len(df):.4f}")
    print("  rollback_rate:", f"{rollback_used/max(1,ridge_used):.4f}" if ridge_used > 0 else "0.0000")
